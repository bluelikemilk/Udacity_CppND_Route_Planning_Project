...
void Render::Display( io2d::output_surface &surface )
{
    ...
    // 3 functions added by udacity 
    // draws a path on the map which connect a vector of nodes that returned by A* search
    DrawPath(surface); 
    // draw point on the map which representing the start/end positon of A*s search
