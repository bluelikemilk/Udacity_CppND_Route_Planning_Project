...
public:
    ...
    // Model constructor accpets a vector map data
...
private:
    // private functions
    void AdjustCoordinates(); // convert log and lat into distance in meters
    void BuildRings( Multipolygon &mp ); // takes polygon data and stores it in the model
    void LoadData(const std::vector<std::byte> &xml); // takes in the data and reads it in all the objects of the model
